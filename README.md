...
This means that a MADA can take 5 stacks of 6 cheap N52 magnets with spacers removed ($25 total from Amazon; each stack of 6 is ~3T) to **~15+T B_opposing** at the *distance* that is ~6 times greater.  12 of the same magnets at each position is **~30+T B_opposing** using 2 $25 Amazon set per MADA.  Put that inside a magnetic circuit in opposition to other identical adjacent MADA, and the B_opposing would be massive. That's before even factoring in additional B_opposing from the partially hybridized MADA pole positions.
